...
public class GetClinicByIdUseCase {
        ...
        public GetClinicDTO execute(Long id, Boolean includePhotos, Boolean includeEquipments,
                ...
                List<GetClinicPhotoDTO> photoDTOs =
                                includePhotos ? clinicPhotoService.listPhotosByClinicId(id).stream()
                                                .map(GetClinicPhotoDTO::new).toList() : null;

                List<GetClinicEquipmentDTO> equipmentDTOs = includeEquipments
                                ? clinicEquipmentService.getEquipmentsByClinicId(id).stream()
                                                .map(GetClinicEquipmentDTO::new).toList()
                                : null;

                List<GetClinicAvailabilityDTO> availabilityDTOs = includeAvailabilities
